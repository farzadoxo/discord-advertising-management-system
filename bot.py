...
class Logging: 
    ...
    async def bot_log(connected=False,ready=False,resumed=False):
        ...
        if connected == True:
            bot_log_embed.add_field(name='اتصال به api انجام شد ✅',value=f'**📡 Ping {int(client.latency * 1000)}**')
            await bot_log_channel.send(embed=bot_log_embed)

        if ready == True:
            bot_log_embed.add_field(name="آماده کار هستم! 🙂‍↕️",value=":)")
            await bot_log_channel.send(embed=bot_log_embed)
            
        if resumed == True:
            bot_log_embed.add_field(name="دوباره برگشتم ☑️",value="به دلایلی ارتباط با api رو از دست دادیم اما دوباره ارتباطمون رو به دست آوردیم")
            await bot_log_channel.send(embed=bot_log_embed)
